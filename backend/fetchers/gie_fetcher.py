...
def compute_fill_signal(fill_pct: float | None, five_yr_avg: float | None) -> dict:
    ...
    if fill_pct > 90:
        ...
        signal = "NEUTRAL"
        note   = f"Storage {fill_pct:.1f}% — within normal seasonal range (deviation: {deviation:+.1f}pp)."
...
def parse_record(rec: dict) -> dict:
    ...
    return {
        "date":         rec.get("gasDayStartedOn") or rec.get("date"),
        "full_twh":     safe_float(rec.get("full")),
        "trend_twh":    safe_float(rec.get("trend")),
        "inject_twh":   safe_float(rec.get("inject")),
        "withdraw_twh": safe_float(rec.get("withdraw")),
        "working_gas_volume_twh": safe_float(rec.get("workingGasVolume")),
        "fill_pct":     safe_float(rec.get("full")),    # GIE 'full' = fill %
        "five_yr_avg":  safe_float(rec.get("fiveYearAverage")),
        "info":         rec.get("info", ""),
    }
