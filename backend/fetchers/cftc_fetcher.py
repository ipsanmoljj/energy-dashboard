...
BASE_URL    = "https://www.cftc.gov/dea/newcot/fut_disagg_txtonly_{year}.zip"
...
def download_cot_csv(year: int) -> str | None:
    """
    Download and unzip the CFTC disaggregated COT file for a given year.
    Returns the CSV content as a string, or None on failure.
    """
    url = BASE_URL.format(year=year)
    log.info("Downloading CFTC COT data: %s", url)

    try:
        r = requests.get(
            url,
            headers={"User-Agent": "EnergyDashboard/1.0 research@example.com"},
            timeout=30,
            stream=True,
        )
        r.raise_for_status()
        ...
        with zipfile.ZipFile(io.BytesIO(r.content)) as zf:
            # The file inside is usually 'fut_disagg_txtonly_<year>.txt'
            names = zf.namelist()
            log.info("ZIP contents: %s", names)

            csv_name = next(
                (n for n in names if n.endswith(".txt") or n.endswith(".csv")),
...
def run() -> dict:
    ...
    # Try current year, fall back to previous year if not yet published
    current_year = date.today().year
    csv_text     = download_cot_csv(current_year)

    if not csv_text:
        log.warning("Current year data unavailable — trying previous year")
        csv_text = download_cot_csv(current_year - 1)
