...
def download_cot_api(market_names: list[str], limit: int = 8) -> dict[str, list[dict]]:
    ...
    Returns {market_name: [row, ...]} for the most recent weeks.

    API: https://publicreporting.cftc.gov/resource/72hh-3qpy.json
    Docs: https://publicreporting.cftc.gov/Commitments-of-Traders/Disaggregated-Futures-Only/72hh-3qpy
    """
    results = {}
    headers = {"User-Agent": "EnergyDashboard/1.0", "Accept": "application/json"}

    for market_name in market_names:
        log.info("  Querying CFTC API: %s", market_name[:50])
        params = {
        ...
        try:
            r = requests.get(CFTC_API_BASE, params=params, headers=headers, timeout=20)
            r.raise_for_status()
            rows = r.json()
            if rows:
                results[market_name] = rows
                log.info("    Got %d rows, latest: %s",
                         len(rows), rows[0].get("report_date_as_yyyy_mm_dd", "?"))
            else:
                log.warning("    No data returned for: %s", market_name)
        except Exception as exc:
            log.error("    API error for %s: %s", market_name, exc)
