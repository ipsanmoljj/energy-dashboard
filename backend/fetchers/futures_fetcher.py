...
def run() -> dict:
    ...
    for ticker, cfg in FUTURES.items():
        ...
        # Use batch result if available, else fall back to chart endpoint
        if ticker in batch:
            b          = batch[ticker]
            raw_price  = b.get("regularMarketPrice")
            prev_close = b.get("regularMarketPreviousClose") or b.get("regularMarketOpen")
            ...
            market_st  = b.get("marketState", "unknown")
            history    = []  # no history in batch call
        else:
            log.debug("  Batch miss — fetching chart for %s", ticker)
            time.sleep(random.uniform(0.5, 1.2))  # polite delay
            chart = fetch_single_chart(ticker, days=60)

            if chart is None:
                log.error("  Could not fetch %s", ticker)
            ...
            high_52w   = chart["high_52w"]
            low_52w    = chart["low_52w"]
            market_st  = chart["market_state"]
