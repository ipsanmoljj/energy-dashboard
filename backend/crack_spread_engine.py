...
def compute_321_crack(wti, rbob_bbl, ho_bbl):
    if None in (wti, rbob_bbl, ho_bbl): return None
...
def get_seasonal_context(month):
    contexts = {
        ...
        11: {"phase": "WINTER_PEAK",       "dominant_product": "heating_oil", "note": "Peak heating oil demand risk. Distillate stocks critical."},
...
def compute_crack_signals() -> dict:
    ...
    # Sanity check BG=F: Yahoo often returns stale low prices
    # Real ICE Gasoil is $500-1200/MT → $67-161/bbl. Below $50/bbl = bad data.
    if ice_gasoil_bbl and ice_gasoil_bbl < 50:
        log.warning("  BG=F price $%.2f/bbl too low (stale Yahoo data) — ignoring", ice_gasoil_bbl)
        ice_gasoil_bbl = None
    ...
    log.info("  ICE GO:   %s $/bbl",    f"{ice_gasoil_bbl:.2f}" if ice_gasoil_bbl else "N/A (stale/unavailable)")
    log.info("─" * 60)
    ...
    gasoil_crack   = (safe(futures, "derived", "gasoil_crack", "value_bbl")
                      or compute_ho_crack(ice_gasoil_bbl, brent))
    ...
    output = {
        ...
        "spreads": {
            "crack_321": {
                ...
                "signal":        ("BULLISH" if crack_321 and crack_321 > 20
                                  else "BEARISH" if crack_321 and crack_321 < 12
                                  else "NEUTRAL"),
                "note": (
                    f"3-2-1 crack ${crack_321:.1f}/bbl: "
                    + ("Wide — product demand outpacing crude. Refiners run harder → crude demand grows."
                ...
                ) if crack_321 else "Insufficient data (WTI or HO price unavailable)",
            },
            "gasoline_crack": {
                ...
                "signal":        ("BULLISH" if gasoline_crack and gasoline_crack > 22
                                  else "BEARISH" if gasoline_crack and gasoline_crack < 10
                                  else "NEUTRAL"),
            },
            "gasoil_crack_ice": {
                ...
                "signal":     ("BULLISH" if gasoil_crack and gasoil_crack > 25
                               else "BEARISH" if gasoil_crack and gasoil_crack < 10
                               else "NEUTRAL"),
                "note": (f"ICE Gasoil crack ${gasoil_crack:.1f}/bbl"
            ...
            "ho_crack": {
                ...
                "signal":     ("BULLISH" if ho_crack and ho_crack > 25
                               else "BEARISH" if ho_crack and ho_crack < 10
                               else "NEUTRAL"),
            },
            "ho_rbob_spread": {
                "value_bbl": ho_rbob_spread,
                "signal":    ("DIESEL_TIGHT"   if ho_rbob_spread and ho_rbob_spread > 15
                              else "GASOLINE_TIGHT" if ho_rbob_spread and ho_rbob_spread < 0
                              else "NORMAL"),
                "note": (
            ...
            "brent_wti": {
                ...
                "signal":     ("ALERT_HIGH" if brent_wti and brent_wti > 8
                               else "ALERT_LOW" if brent_wti and brent_wti < 2
                               else "NORMAL"),
                "alert":      bwti_alert,
                "note": (
        ...
        "nci_crack": {
            ...
            "interpretation":  (
                ...
                + {
                    "CRACKS_VERY_WIDE":   "Exceptional refinery margins. Strong crude demand pull.",
                    "CRACKS_WIDE":        "Above-average margins. Bullish crude demand.",
                    "CRACKS_NORMAL_HIGH": "Margins slightly above seasonal norm. Mild bullish.",
                    "CRACKS_BALANCED":    "Margins near seasonal average. Neutral signal.",
                    "CRACKS_COMPRESSED":  "Below-average margins. Risk of run cuts. Mild bearish.",
                    "CRACKS_NARROW":      "Margins compressed. Refiners cutting runs. Bearish.",
                    "CRACKS_VERY_NARROW": "Margins very low. Significant run cuts likely.",
                }.get(crack_label, "")
            ),
        },
    ...
    log.info("3-2-1 crack:       %s $/bbl", f"{crack_321:.1f}" if crack_321 else "N/A")
    log.info("Gasoline crack:    %s $/bbl", f"{gasoline_crack:.1f}" if gasoline_crack else "N/A")
    log.info("HO-RBOB spread:    %s $/bbl", f"{ho_rbob_spread:.1f}" if ho_rbob_spread else "N/A")
