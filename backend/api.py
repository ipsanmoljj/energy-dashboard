...
// ── TabSpreads — now includes quality spreads ─────────────────────────────────
function TabSpreads({ d, history }) {
  ...
  const catLabel = c =>
    c === "light_heavy" ? "Light-Heavy" :
    c === "sweet_sour"  ? "Sweet-Sour"  : "Product"

  const catColor = c =>
    c === "light_heavy" ? "#a78bfa" :
    c === "sweet_sour"  ? "#f59e0b" : "#22c55e"
  ...
  return (
    <>
      ...
      {qsList.length > 0 && (
        <Card title="Quality & Grade Spreads" style={{ marginBottom: 12 }}>
              ...
              <div key={i} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  <div>
                    ...
                    }}>{catLabel(s.category)}</span>
                  </div>
                  <Badge label={s.signal} />
                </div>
                <div style={{ fontSize: 20, fontWeight: 800, color: s.value >= 0 ? "#e5e7eb" : "#ef4444", lineHeight: 1 }}>
                  {s.value != null ? `${s.value >= 0 ? "+" : ""}${s.value.toFixed(2)}` : "—"}
                  <span style={{ fontSize: 10, color: "#4b5563", marginLeft: 4 }}>$/bbl</span>
                </div>
                <div style={{ fontSize: 9, color: "#374151", marginTop: 4, lineHeight: 1.3 }}>
                  {s.interpretation?.slice(0, 80)}
                </div>
              </div>
            ))}
          </div>
          {qs.methodology && (
            <div style={{ fontSize: 9, color: "#1f2937", marginTop: 10, fontStyle: "italic" }}>
              {qs.methodology}
            </div>
          )}
        </Card>
      ...
      <Card title="Signal Reference">
        {[
          ...
          <div key={i} style={{ display:"flex", gap:8, padding:"5px 0", borderBottom:"1px solid #0f1e30", fontSize:11 }}>
            <span style={{ color:"#f59e0b", fontWeight:700, minWidth:170 }}>{k}</span>
...
export default function App() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [all, eia, rig, crack, hist, invSig, crackSig, fj, qs] = await Promise.all([
      ...
      const merged = { ...all, eia, rig_count: rig, crack, inv_signals: invSig, crack_signals: crackSig, fj, quality_spreads: qs }
  ...
  return (
    <div style={{ background:"#060d18", minHeight:"100vh", color:"#e5e7eb",
      ...
      <div style={{ textAlign:"center", padding:"10px 0", color:"#1a2535",
        ...
        EIA · YAHOO FINANCE · FRED · GIE AGSI+ · OPEN-METEO · CFTC · BAKER HUGHES · ARGUS/PLATTS
      </div>
    </div>
  )
}
